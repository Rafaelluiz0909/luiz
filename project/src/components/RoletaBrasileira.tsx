...
import { CircleDot, HelpCircle, BarChart2, Flame } from 'lucide-react';
...
export function RoletaBrasileira() {
  ...
  const addFloatingWindow = (type: 'stats' | 'hotcold') => {
    ...
    <div className="bg-zinc-800 rounded-lg shadow-xl p-3 md:p-4 border border-red-600/20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 space-y-2 md:space-y-0">
        <div className="flex items-center gap-2">
          <h2 className="text-lg md:text-xl font-bold text-gray-100">Roleta Brasileira</h2>
          <button
      ...
      {floatingWindows.map((window) => (
        <FloatingWindow
          key={window.id}
          id={window.id}
          title={window.type === 'stats' ? 'Estatísticas dos Números' : 'Números Quentes e Frios'}
